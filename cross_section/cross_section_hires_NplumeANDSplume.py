...
FIGDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/MS/crosssection/spinup/bothplumes/'
...
# Don't really want to make plots for every time, just starting on June 18th
...
for t in np.arange(startplot_index,int(np.shape(nc.variables['Times'])[0])):
    ...
    #               PH = level-staggered
    #               TH = level
    # Right now I don't deal with this, they are close enough for bottom levels, 
    # so plot the bottom x levels
    b_levels = 12
    PH_b = TPH[:b_levels]
    TH_b = TTH[:b_levels]
