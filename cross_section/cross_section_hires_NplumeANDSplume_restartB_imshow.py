...
##Subplot 2####################################################################################################### Subplot 2    
    ...
    for i in range(0,np.shape(N_SLV_b)[0]):
        for j in range(0,np.shape(N_SLV_b)[1]):
            plt.text(j+.2,i+.2,'%.2f' % float(N_SLV_b[i,j]),fontsize=7)
...
######################################################################################################### Subplot 2    
    ...
    plt.savefig(FIGDIR+time_save+'__North_VALUES.png', bbox_inches='tight',dpi=100)
    print 'Saved'
