...
FIGDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/MS/crosssection/spinup/Splume/vectors/'
...
for t in np.arange(startplot_index,int(np.shape(nc.variables['Times'])[0]),6):
    ...
    if time_dt.minute==0:
        ...
        # Add vectors
        #limit plotted speeds to 50 cm/s
        Vlim = np.ma.masked_array(Vavg,Vavg<50)
        Wlim = np.ma.masked_array(Wavg,Wavg<50)
        plt.quiver(LAT2D,PH_b,Vlim,Wlim,
                   color='darkgreen',
                   width=0.0022,
                   scale=10000)        
