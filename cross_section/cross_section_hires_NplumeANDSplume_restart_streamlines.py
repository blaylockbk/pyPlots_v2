...
FIGDIR = '/uufs/chpc.utah.edu/common/home/u0553130/public_html/MS/crosssection/spinup/bothplumes/restart/streamlines/'
if not os.path.exists(FIGDIR):
    os.makedirs(FIGDIR)

RESTARTDIR = '/uufs/chpc.utah.edu/common/home/horel-group4/model/bblaylock/WRF3.7_spinup/DATA/RESTART_June18-12:00--19-07:00/'
RESTARTAUX = 'auxhist24_d02_2015-06-18_12:00:00'
...
startplot_index = int(144*4.5)  # add one day = index 144 ## Oh, I need to use nc.variables['Times']
...
ncRESTART = netcdf.netcdf_file(RESTARTDIR+RESTARTAUX,'r')
...
for t in np.arange(startplot_index,int(np.shape(nc.variables['Times'])[0])):    
    ...
    if time_dt.minute==0:    
        ...
        if time_dt.minute == 0:
            ...
            V = ncWRFout.variables['V'][0][:,lat,lon]
            Vl = ncWRFout.variables['V'][0][:,lat,lon-1]
            Vr = ncWRFout.variables['V'][0][:,lat,lon+1]
            Vavg = ((V+Vl+Vr)/3)*100
        ...
        # theta left
        THl  = nc.variables['T'][t][:,lat,lon-1]    # Perturbation potential tempertaure [K]
        TTHl = TH+300                          # Total Potential Temperature (theta) [K]
        # theta right    
        THr  = nc.variables['T'][t][:,lat,lon+1]    # Perturbation potential tempertaure [K]
        ...
        # Plume Data
        N_SLV   = ncRESTART.variables['N_SLV_rst'][t-648][:,lat,lon]
        N_SLV_l = ncRESTART.variables['N_SLV_rst'][t-648][:,lat,lon-1]
        N_SLV_r = ncRESTART.variables['N_SLV_rst'][t-648][:,lat,lon+1] 
        S_SLV   = ncRESTART.variables['S_SLV_rst'][t-648][:,lat,lon]
        S_SLV_l = ncRESTART.variables['S_SLV_rst'][t-648][:,lat,lon-1]
        S_SLV_r = ncRESTART.variables['S_SLV_rst'][t-648][:,lat,lon+1]
        ...
        ### streamlines can only be plotted on a rectangular grid.
        ### So we'll replace the LAT2D and variable with something
        ### the same size as S_SLV_b, N_SLV_b, Uavg_b, and Wavg_b
        nx,ny = np.shape(LAT2D)
        y = np.linspace(1,ny,ny)
        x = np.linspace(1,nx,nx)
        X,Y = np.meshgrid(y,x)    
        
        #plt.contourf(LAT2D,Y,S_SLV_b,
        #             cmap=cmapblue,
       ...
       #            fontsize=7,fmt = '%1.0f')
        ...
        #WW = plt.contour(LAT2D,Y,Wavg,
        #                 colors='darkblue',
        #                 levels=contour_levels,
        #                 linewidths=1.2)
        #WW = plt.clabel(WW, WW.levels[::2],fontsize=9, inline=1,fmt = '%1.0f')
        
        speed = np.sqrt(Wavg*Wavg + Vavg*Vavg)  
        plt.streamplot(LAT2D,Y,Vavg,Wavg, color=speed,density=0.6, cmap=plt.cm.autumn)
        ## ----- Vertical Wind Speed-----------------------------------------------    
        ...
        print 'Saved', str_time, 'max plume', np.max(N_SLV_b)
